...
function DailyReports() {
  ...
  const dailyReportData = blogData.filter(item => item.category === 'Daily Report');
  function updateMetaTags() {
    ...
    let title = "FiYDAA Insights | Latest Daily Reports from FiYDAA";
    let description = "Top daily reports from the world of gold investments and stock market for first-time and experienced investors";
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            {dailyReportData.slice(0, visibleCount).map((report, index) => {
              ...
              return (
                ...
                >
                  <div key={index} className="bg-[#F2F3F5] flex flex-col h-[450px] overflow-hidden w-5/6 justify-center mb-20 ml-10 shadow-lg">
                    <img
                    ...
                    <div className='p-4 flex flex-col justify-between flex-grow overflow-y-auto'>
                      <div>
                        <h1 className="font-outfit font-bold text-lg lg:text-xl mb-2 font-poppins">
                          {report.title}
                        </h1>
                        <p className="text-gray-500  font-outfit text-sm mb-4 font-poppins">
                          {report.content.split(" ").length > 30
                            ? `${report.content.split(" ").slice(0, 30).join(" ")}...`
                            : report.content}
                        </p>
                      </div>
                      <div className="mt-auto">
                        <div className="flex justify-between items-center">
                          <span className="font-outfit text-gray-500 text-sm font-poppins">
                            {formattedDate}
                          </span>

                          <Link to={report.read_more_link} target='_blank' className="font-outfit text-black cursor-pointer font-bold font-poppins">Read More</Link>
